...
const Navbar = () => {
  ...
  return (
    <nav className="h-[80px] z-10 px-6 w-screen border-b-2 bg-white dark:!bg-neutral-900 md:px-24" style={user ? { position:'fixed'} : {position: 'static'} }>
      <div className="container mx-auto flex items-center justify-around h-full">
        <h1 className="name-app font-bold text-xl drop-shadow-md md:text-2xl lg:text-4xl">
          Codigram
        </h1>
        ...
        <div className="text-2xl flex items-center ">
          {user ? (
            <ul className="flex space-x-3.5 items-center justify-center text-sm md:mr-10 md:text-base">
              ...
              >
                <button type="button" className="cursor-pointer font-semibold hover:text-indigo-500">Logout</button>
              </li>
            </ul>
          ) : (
            <ul className="flex space-x-3.5 items-center justify-center text-sm md:mr-10 md:text-base">
